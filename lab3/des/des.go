...
func (d *Des) GetKey() uint64 {
	return d.key
}

func (d *Des) keyPermutation56bTo28b() (uint32, uint32) {
	var k1, k2 uint64

	// 1011 0100 >> 4 = 0000 1011 & 1 = 0000 0001 << 4 = 0001 0000
	for i := 0; i < 28; i++ {
		k1 |= ((d.key >> (64 - K1P[i])) & 1) << (31 - i)
		k2 |= ((d.key >> (64 - K2P[i])) & 1) << (31 - i)
	}
	return uint32(k1), uint32(k2)
}

func (d *Des) keyExpansion() [RoundCount]uint64 {
	var keys48b [RoundCount]uint64
	k1, k2 := d.keyPermutation56bTo28b()
	...
	for i := 0; i < RoundCount; i++ {
		...
		for j := 0; j < 48; j++ {
			keys48b[i] |= ((block56b >> (64 - CP[j])) & 1) << (63 - j)
		}
	}
